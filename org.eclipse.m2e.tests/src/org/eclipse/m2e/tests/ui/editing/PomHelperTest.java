...
public class PomHelperTest extends TestCase {
	...
	public void testCreateDependency() {
		...
		PomHelper.createDependency(findChild(tempModel.getDocument().getDocumentElement(), "dependencies"), dep.getGroupId(), dep.getArtifactId(), dep.getVersion());

		assertNotNull(PomHelper.findDependency(tempModel.getDocument(), dep));
	}

	public void testCreateDependencyInEmptyList() {
		tempModel.getStructuredDocument().setText(StructuredModelManager.getModelManager(), "<project><dependencies/></project>");
	...
	public void testCreatePlugin() {
		...
		PomHelper.createPlugin(findChild(tempModel.getDocument().getDocumentElement(), "plugins"), dep.getGroupId(), dep.getArtifactId(), dep.getVersion());
	...
	public void testCreatePluginInEmptyList() {
		tempModel.getStructuredDocument().setText(StructuredModelManager.getModelManager(), "<project><plugins/></project>");
	...
	private static Element findPlugin(Element parent, ArtifactKey key) {
		return findChild(parent, "plugin", PomEdits.childEquals("groupId", key.getGroupId()), PomEdits.childEquals("artifactId", key.getArtifactId()),
				PomEdits.childEquals("version", key.getVersion()));
	}
}

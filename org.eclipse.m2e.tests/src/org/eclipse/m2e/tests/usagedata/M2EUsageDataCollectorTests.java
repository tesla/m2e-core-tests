...
public class M2EUsageDataCollectorTests extends AbstractMavenProjectTestCase {
	...
	public void setUp() throws Exception {
		...
		Thread.sleep(1000);
		Field monitorField = Activator.class.getDeclaredField("monitor");
		monitorField.setAccessible(true);
		monitor = (PomImportMonitor) monitorField.get(Activator.getDefault());
	...
	public void testPluginEvent() throws Exception {
		...
		assertContains("Plugin Event", "clean:default-clean:clean", "m2e.plugins", "org.apache.maven.plugins:maven-clean-plugin:2.4.1", "org.eclipse.m2e.core", "0.13.0");

	}

	private void assertContains(String message, String what, String kind, String description, String bundleId, String bundleVersion) {
		StringBuilder e = new StringBuilder();
		for (UsageDataEvent event : usageDataService.events) {
			if (event.what.equals(what) && event.kind.equals(kind) && event.description.equals(description) && event.bundleId.equals(bundleId) && event.bundleVersion.equals(bundleVersion)) {
				return;
			}
	...
	private static String toString(UsageDataEvent event) {
		return new StringBuilder().append(event.what).append(',').append(event.kind).append(',').append(event.description).append(',').append(event.bundleId).append(',').append(event.bundleVersion)
				.toString();
	}

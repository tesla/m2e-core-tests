  ...
  static class GoalsContentProvider implements ITreeContentProvider {
    ...
    public GoalsContentProvider() {
      ...
      try {
        ...
        MavenPlugin.log("Unable to get lifecycle phases", e);
      }
      ...
      try {
        ...
        MavenPlugin.log("Unable to get plugin list", e);
      }
    }
  ...
  static class GoalsSelectionValidator implements ISelectionStatusValidator {
    public IStatus validate(Object[] selection) {
      if(selection.length == 0) {
        return new Status(IStatus.ERROR, MavenPlugin.PLUGIN_ID, -1, "Should select at least one goal", null);
      }
      for(int j = 0; j < selection.length; j++ ) {
        if(selection[j] instanceof Entry) {
          continue;
        }
        return new Status(IStatus.ERROR, MavenPlugin.PLUGIN_ID, -1, "", null);
      }
